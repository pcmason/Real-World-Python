...
class Search():
    ...
    def __init__(self, name):
        ...
        # Challenge 3 - Placeholder attributes for probability of detection
        self.pod1 = 0
        self.pod2 = 0
        self.pod3 = 0
    ...
    def calc_search_effectiveness(self):
        ...
        # Challenge 3 - Change distribution method from uniform to triangular to get a more standard distribution
        self.sep1 = random.triangular(0.2, 0.9)
        self.sep2 = random.triangular(0.2, 0.9)
        self.sep3 = random.triangular(0.2, 0.9)
        # Challenge 3 - Now set the values for each pod as well
        self.pod1 = self.sep1 * self.p1
        self.pod2 = self.sep2 * self.p2
        self.pod3 = self.sep3 * self.p3
    ...
    def revise_target_probs(self):
        ...
        # Add this so that there is no division by 0 error
        if denom == 0:
            denom = 0.000000001
    ...
    def draw_menu(self, search_num):
        ...
        # Challenge 3 - Calculate the actual pods for each specific search
        pod1_twice = 1 - (1 - self.pod1)**2
        pod2_twice = 1 - (1 - self.pod2)**2
        pod3_twice = 1 - (1 - self.pod3)**2
        pod1_pod2 = self.pod1 + self.pod2
        pod2_pod3 = self.pod2 + self.pod3
        pod1_pod3 = self.pod1 + self.pod3
        ...
        print(
            ...
            6 - Search Areas 2 & 3             
              Probability of detection: %.3f
            7 - Start Over
            """ % (pod1_twice, pod2_twice, pod3_twice, pod1_pod2, pod1_pod3, pod2_pod3)
        )
...
"""Challenges:
    ...
    the menu, for searching an area twice pod = 1 - (1- pod)^2 [COMPLETE]
    
    Steps made to complete:
        - First will comment out the simulation stuff for Challenge 2. Make the user play the game again
        - Seems like it should be pretty simple, create 3 more class variables for each SA to represent the pod. Then
          update the value for each pod based on the menu option chosen. 
        - Change the method used to generate the random sep from uniform() to triangular() for a more standard dist.
"""
